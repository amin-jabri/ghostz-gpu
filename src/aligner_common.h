...
class AlignerCommon {
 public:
  ...
  static void BuildResult(Query &query, TDatabase &database,
                          uint32_t database_chunk_id,
                          uint32_t subject_id_in_chunk, int score,
                          Coordinate &hit, Coordinate &start, Coordinate &end,
                          std::vector<EditBlocks::EditOpType> &edits,
                          Result &result);

  template <typename TDatabase>
  static void WriteOutput(std::ostream &os, Queries &queries,
                          TDatabase &database,
                          AligningCommonParameters &parameters,
...
template <typename TDatabase>
void AlignerCommon::AddResults(TDatabase &database,
                               AligningCommonParameters &parameters,
                               std::vector<std::vector<AlignmentPosition> >
                                   &alignment_start_positions_in_database,
                               std::vector<PresearchedResult> &added_results,
  ...
  // sort(added_results.begin(), added_results.end(),
...
///////////////////////////////////////////////////////////
  ...
  std::vector<PresearchedResult>::iterator added_results_iter =
      added_results.begin();
  std::vector<PresearchedResult>::iterator added_results_end =
      added_results.end();
  while (new_results.size() < parameters.max_number_results &&
    ...
    if (added_results_iter == added_results_end) {
      ...
      assert(added_alignment.database_chunk_id <
      ...
      ++results_iter;
               ...
               AlignmentComp(results_iter->score, results_iter->start,
                             ...
                             added_results_iter->start,
                             added_results_iter->end) < 0) {
      added_alignment.database_chunk_id = database.GetChunkId();
      added_alignment.subject_id_in_chunk =
          database.GetId(added_results_iter->start.database_position);
      assert(added_alignment.subject_id_in_chunk <
             database.GetNumberSequencesInChunk());
    ...
    if (added_alignment.database_chunk_id == database.GetChunkId()) {
      if (alignment_start_positions_in_database[added_alignment
                                                    .subject_id_in_chunk]
              .size() < parameters.max_number_one_subject_results) {
        std::vector<AlignmentPosition> *start_positions =
            &alignment_start_positions_in_database[added_alignment
                                                       .subject_id_in_chunk];
        AlignmentPosition searched_position;
        searched_position.result_id = 0;
        searched_position.position = added_alignment.start.database_position;
        std::vector<AlignmentPosition>::iterator start_positions_iter =
            lower_bound(start_positions->begin(), start_positions->end(),
                        searched_position, AlignmentPositionLessPosition());
        std::vector<AlignmentPosition>::iterator start_positions_insert_iter =
            start_positions_iter;
        searched_position.position = added_alignment.end.database_position;
        std::vector<AlignmentPosition>::iterator start_positions_iter_end =
            lower_bound(start_positions_iter, start_positions->end(),
                        searched_position, AlignmentPositionLessPosition());
        ...
        for (; start_positions_iter != start_positions_iter_end;
          ...
          if (IsContained(r_ptr->start, r_ptr->end, added_alignment.start,
                          added_alignment.end)) {
            added = false;
            break;
          }
        ...
        if (added == true) {
          ...
          alignment_start_positions_in_database[added_alignment
                                                    .subject_id_in_chunk]
              .insert(start_positions_insert_iter, p);
...
void AlignerCommon::BuildResult(Query &query, TDatabase &database,
                                uint32_t database_chunk_id,
                                uint32_t subject_id_in_chunk, int score,
                                Coordinate &hit, Coordinate &start,
                                Coordinate &end,
                                std::vector<EditBlocks::EditOpType> &edits,
                                Result &result) {
...
/////////////////////////////////////////////////////////////
  ...
  result.start.database_position =
      start.database_position - (database.GetOffset(subject_id_in_chunk) - 1);
  result.end.database_position =
      end.database_position - (database.GetOffset(subject_id_in_chunk) - 1);
}

template <typename TDatabase>
void AlignerCommon::WriteOutput(std::ostream &os, Queries &queries,
                                TDatabase &database,
                                AligningCommonParameters &parameters,
                                std::vector<std::vector<Result> > &results) {
  ...
  statistics.CalculateGappedKarlinParameters(
  ...
  for (uint32_t i = 0; i < queries.GetNumberOfSequences(); ++i) {
    ...
    uint64_t search_space = statistics.CalculateSearchSpace(
        query->GetRealSequenceLength(), database.GetDatabaseTotalLenght(),
        database.GetNumberTotalSequences(), gapped_karlin_parameters, alpha,
        beta);
    for (std::vector<Result>::iterator it = results[i].begin();
         it != results[i].end(); ++it) {
      os << query_name << "\t";
      os << it->subject_name << "\t";
      os << (1.0 - (static_cast<float>(it->mismatches + it->gap_openings +
                                       it->gap_extensions) /
      ...
      os << Statistics::Nominal2EValue(it->score, search_space,
                                       gapped_karlin_parameters)
         << "\t";
      os << Statistics::Nominal2Normalized(it->score, gapped_karlin_parameters);
