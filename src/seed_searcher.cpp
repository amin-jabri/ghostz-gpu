...
SeedSearcher::SeedSearcher()
    : used_clustering_(true),
      sequence_delimiter_(0),
      hit_similarity_threshold_rate_(0.0f),
      number_of_threads_(1),
      subsequence_length_(0),
      max_representation_member_distance_(0),
      distance_threshold_(0),
      database_parameters_ptr_(NULL),
      query_parameters_ptr_(NULL),
      sequences_index_(NULL),
      clustering_sequences_index_(NULL),
      query_seed_list_offsets_(NULL),
      hash_position_data_lists_(NULL),
      query_sequences_(NULL),
      database_sequence_(NULL),
      score_matrix_(NULL),
      ungapped_extension_cutoffs_(NULL),
      gapped_extension_triggers_(NULL)
#if 1
      ,
      hash_hits_count(0),
      calculate_distance_count(0),
      filter_out_from_distance_count(0),
      representation_hit_count(0),
      representation_no_similarity_check_hit_count(0),
      member_hit_count(0),
      member_no_similarity_check_hit_count(0),
      ungapped_extension_count(0)
#endif
{
}
SeedSearcher::~SeedSearcher() {}

uint32_t SeedSearcher::Reset(
    SeedSearcherQueryParameters &query_parameters,
  ...
  ungapped_extension_cutoffs_ = &query_parameters.GetUngappedExtensionCutoffs();
  gapped_extension_triggers_ = &query_parameters.GetGappedExtensionTriggers();
  return 0;
}

bool SeedSearcher::Search(
    const uint32_t hash_position_data_list_i,
    std::vector<uint32_t> &hitting_query_position_data_i_list,
    TempChainFilterSeedList *temp_ungapped_extension_seed_data) {
  const SeedSearcherQueryParameters::HashPositionDataList
      &hash_position_data_list =
          (*hash_position_data_lists_)[hash_position_data_list_i];

  SearchFromClusteringSubsequences(
      *query_sequences_, database_sequence_, sequence_delimiter_,
      hash_position_data_list, *clustering_sequences_index_,
      subsequence_length_, *redueced_code_map_,
      max_representation_member_distance_, distance_threshold_, *score_matrix_,
      *ungapped_extension_cutoffs_, *gapped_extension_triggers_,
      hitting_query_position_data_i_list, temp_ungapped_extension_seed_data);

  SearchFromNonClusteringSubsequences(
      *query_sequences_, database_sequence_, sequence_delimiter_,
      hash_position_data_list, *sequences_index_, *score_matrix_,
      *ungapped_extension_cutoffs_, *gapped_extension_triggers_,
...
int SeedSearcher::SearchFromClusteringSubsequences(
    ...
    const SeedSearcherQueryParameters::HashPositionDataList
        &hash_position_data_list,
    const SeedSearcherDatabaseParameters::ClusteringSequencesIndex
        &clustering_sequences_index,
  ...
  const SeedSearcherDatabaseParameters::ClusteringSequencesIndex::
      RepresentativeData *representatives;
...
#if DEBUG
    hash_hits_count +=
        representatives_length * hash_position_data_list.position_data.size();
#endif

    for (size_t representatives_i = 0;
         representatives_i < representatives_length; ++representatives_i) {
      ...
      for (size_t query_position_data_i = 0;
        ...
        const SeedSearcherQueryParameters::HashPositionData
            &query_position_data =
        ...
        if (query_position_data.similarity_check) {
          ...
          query_representation_distance = SeedSearcherCommon::CalculateDistance(
              query_sequences[query_position_data.query_id] +
                  query_position_data.position,
              representative_subsequence_center, subsequence_length,
              redueced_code_map);

          if (IsSufficientSimilarityCluster(query_representation_distance,
                                            max_representation_member_distance,
                                            distance_threshold)) {
            hitting_query_position_data_i_list.push_back(query_position_data_i);
          }
        } else {
          hitting_query_position_data_i_list.push_back(query_position_data_i);
        }
...
#endif
          int score = UngappedExtend(
              query_sequences[query_position_data.query_id],
              query_position_data.position, database_sequence,
              representative_position, sequence_delimiter, score_matrix,
              ungapped_extension_cutoffs[query_position_data.query_id],
              gapped_extension_triggers[query_position_data.query_id]);
          if (score > gapped_extension_triggers[query_position_data.query_id]) {
            AddSeed(query_position_data.query_id, query_position_data.position,
                    representative_position, temp_ungapped_extension_seed_data);
          }
        }
      }

      const SeedSearcherDatabaseParameters::ClusteringSequencesIndex::Position
          *members;
      ...
      if (!hitting_query_position_data_i_list.empty() &&
          !clustering_sequences_index.GetMembers(
              ...
              &members_length)) {
        for (size_t members_i = 0; members_i < members_length; ++members_i) {
          for (size_t i = 0; i < hitting_query_position_data_i_list.size();
               ++i) {
            const SeedSearcherQueryParameters::HashPositionData
                &query_position_data =
                    hash_position_data_list
                        .position_data[hitting_query_position_data_i_list[i]];
#if DEBUG
            if (query_position_data.similarity_check) {
              ++member_hit_count;
              ++hash_hits_count;
            ...
            int score = UngappedExtend(
                query_sequences[query_position_data.query_id],
                query_position_data.position, database_sequence,
                members[members_i], sequence_delimiter, score_matrix,
                ungapped_extension_cutoffs[query_position_data.query_id],
                gapped_extension_triggers[query_position_data.query_id]);
            if (score >
                gapped_extension_triggers[query_position_data.query_id]) {
              AddSeed(query_position_data.query_id,
                      query_position_data.position, members[members_i],
                      temp_ungapped_extension_seed_data);
            }
          }
        }
...
#if DEBUG
    hash_hits_count +=
        values_length * hash_position_data_list.position_data.size();
#endif
    for (size_t values_i = 0; values_i < values_length; ++values_i) {
      for (std::vector<SeedSearcherQueryParameters::HashPositionData>::
               const_iterator queries_position_data_it =
        ...
        int score = UngappedExtend(
            query_sequences[queries_position_data_it->query_id],
            queries_position_data_it->position, database_sequence,
            values[values_i], sequence_delimiter, score_matrix,
        ...
        if (score >
          ...
          AddSeed(queries_position_data_it->query_id,
                  queries_position_data_it->position, values[values_i],
                  temp_ungapped_extension_seed_data);
        }
      }
    }
...
void SeedSearcher::AddSeed(
    uint32_t query_id, uint32_t query_position, uint32_t database_position,
...
bool SeedSearcher::IsSufficientSimilarityCluster(
  ...
  int lower_bound = max(
      query_representation_distance - max_representation_member_distance, 0);
