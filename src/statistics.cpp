...
float Statistics::Nominal2Normalized(int normal_score,
                                     const KarlinParameters &paramters) {
  float normalized_score;
  normalized_score = (((static_cast<float>(normal_score) * paramters.lambda) -
                       log(paramters.K)) /
                      static_cast<float>(log(2.0)));
  return normalized_score;
}
int Statistics::Normalized2Nominal(float normalized_score,
                                   const KarlinParameters &paramters) {
  float normal_score;
  normal_score =
      (normalized_score * static_cast<float>(log(2.0)) + log(paramters.K)) /
      paramters.lambda;
  return static_cast<int>(floor(normal_score));
}

int Statistics::NormalizedCutoff2NominalCutoff(
    float normalized_cutoff, const KarlinParameters &paramters) {
  float normal_cutoff;
  normal_cutoff =
      (normalized_cutoff * static_cast<float>(log(2.0))) / paramters.lambda;
  return static_cast<int>(floor(normal_cutoff));
}

double Statistics::Nominal2EValue(int normal_score, uint64_t search_space,
                                  const KarlinParameters &paramters) {
  double e_value;
  e_value = search_space * paramters.K *
            exp(static_cast<double>(-1.0 * normal_score * paramters.lambda));
  return e_value;
}

uint64_t Statistics::CalculateSearchSpace(uint64_t query_length,
                                          uint64_t db_length,
                                          uint64_t db_number_sequences,
                                          const KarlinParameters &parameters,
                                          double alpha, double beta) {
  uint64_t length_adjustment = CalculateLengthAdjustment(
      query_length, db_length, db_number_sequences, parameters, alpha, beta);
  uint64_t effective_db_length =
      db_length - (db_number_sequences * length_adjustment);
  return effective_db_length * (query_length - length_adjustment);
}

uint64_t Statistics::CalculateLengthAdjustment(
    uint64_t query_length, uint64_t db_length, uint64_t db_number_sequences,
    const KarlinParameters &paramters, double alpha, double beta) {
  ...
  if (typeid(*type_) == typeid(ProteinType)) {
    ...
    for (uint32_t i = 0; i < 5; ++i) {
      length = (paramters.K + log((query_length - length) *
                                  (db_length - db_number_sequences * length))) /
               paramters.H;
...
bool Statistics::CalculateUngappedIdealKarlinParameters(
  ...
  bool ret;
  ret = CalculateScoreProbabilities(score_matrix, &letter_prob_[0],
                                    &letter_prob_[0], score_probabilities);
  if (!ret) {
    return false;
  }
  BlastKarlinBlkCalc(score_probabilities, lowest, highest, &(paramters->lambda),
                     &(paramters->K), &(paramters->H));
  return true;
}

bool Statistics::CalculateUngappedKarlinParameters(
    const AlphabetCoder::Code *sequence, uint32_t length,
    const ScoreMatrix &score_matrix, KarlinParameters *paramters) {
  ...
  vector<double> score_probabilities0(highest - lowest + 1);
  double *score_probabilities = &score_probabilities0[-lowest];
  ret = CalculateScoreProbabilities(score_matrix, &query_frequency[0],
                                    &letter_prob_[0], score_probabilities);
  if (!ret) {
    return false;
  }
...
bool Statistics::CalculateGappedKarlinParameters(
    const ScoreMatrix &score_matrix, int gap_open, int gap_extension,
    KarlinParameters *parameters) {
  if (typeid(*type_) == typeid(DnaType)) {
    ...
    if (score_matrix.GetName() == "BLOSUM62" && gap_open == 11 &&
      ...
      parameters->H = 0.14;
      return true;
    } else if (score_matrix.GetName() == "PAM30" && gap_open == 9 &&
               gap_extension == 1) {
...
bool Statistics::CalculateAlphaBeta(const ScoreMatrix &score_matrix,
...
bool Statistics::CalculateScoreProbabilities(const ScoreMatrix &score_matrix,
                                             double *score_frequency1,
                                             double *score_frequency2,
...
bool Statistics::ComposeSequenceFrequency(const AlphabetCoder::Code *sequence,
                                          uint32_t length, double *frequency) {
