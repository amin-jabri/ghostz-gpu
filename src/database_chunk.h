 ...
 public:
  ...
  bool Build(std::vector<Sequence> &sequences, AlphabetCoder &coder,
             ...
             SeedSearcherParametersBuildParameters
                 &seed_search_parameters_build_parameters);
  bool Clear();
  uint32_t GetNumberSequences() { return number_sequences_; }
  ...
  typename SeedSearcherParameters::CommonParameters &
...
DatabaseChunk<TSeedSearcher>::DatabaseChunk()
    : building_(false),
      ...
      names_(0),
      offsets_(0),
      concatenated_sequence_(0),
      seed_searcher_parameters_() {}
...
template <typename TSeedSearcher>
bool DatabaseChunk<TSeedSearcher>::Build(
    std::vector<Sequence> &sequences, AlphabetCoder &coder,
    AlphabetCoder::Code sequence_delimiter,
    SeedSearcherParametersBuildParameters
        &seed_search_parameters_build_parameters) {
...
template <typename TSeedSearcher>
void DatabaseChunk<TSeedSearcher>::EncodeSequences(
    AlphabetCoder &coder, std::vector<Sequence> &sequences,
    AlphabetCoder::Code sequence_delimiter) {
...
bool DatabaseChunk<TSeedSearcher>::LoadSeedSearcherCommonParameters(
  ...
  if (in) {
    seed_searcher_parameters_.BuildCommonParameters(
        in, &seed_searcher_common_parameters_);
    in.close();
    setted_seed_searcher_common_parameters_ = true;
    return true;
  }
...
bool DatabaseChunk<TSeedSearcher>::SaveNames(std::string filename_prefix) {
  ...
  for (std::vector<std::string>::iterator i = names_.begin(); i != names_.end();
       ++i) {
    out << *i << std::endl;
  }
  out.close();
