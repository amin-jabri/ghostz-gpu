...
int SeedSearcherGpu::SearchSeedsFromClusteringSubsequence(
  ...
  const uint32_t hash_position_data_list_id =
      distance_calculation_seed_list
          .GetHashPositionDataListIds()[hash_position_data_list_ids_i];
  SeedSearcherGpu::QueryParameters::HashPositionDataList
...
#endif
  if (used_clustering_) {
    SearchSeedsFromClusteringSubsequences(
        hash_position_data_list_ids_i,
        ...
        *clustering_sequences_index_, distance_calculation_seed_list,
        ...
        passing_distance_calculation_i_list, ungapped_extension_seed_list);
  }
  return 0;
}
...
int SeedSearcherGpu::SearchSeedsFromRepresentativesForSimilarityFiltering(
    const SeedSearcherGpuQueryParameters::HashPositionDataList
        &hash_position_data_list,
    const DatabaseParameters::ClusteringSequencesIndex
  ...
  if (!clustering_sequences_index.GetRepresentatives(
    ...
    for (size_t representatives_i = 0;
      ...
      for (size_t query_position_data_i = 0;
        ...
        const SeedSearcherGpuQueryParameters::HashPositionData
            &query_position_data =
                hash_position_data_list
                    .check_position_data[query_position_data_i];
#if DEBUG
        ++calculate_distance_count;
#endif
        distance_calculation_seed_list->AddSeed(query_position_data.position,
  ...
  if (!clustering_sequences_index.GetRepresentatives(
    ...
    for (size_t representatives_i = 0;
...
#endif
      const vector<SeedSearcherGpu::QueryParameters::HashPositionData>
          &check_query_position_list =
              hash_position_data_list.check_position_data;
      size_t number_query_position_data = check_query_position_list.size();
      int offset =
          seed_offset + (int)current_seed_i - (int)numer_calculated_seeds;
      ...
      for (size_t i = 0; i < number_query_position_data;
           ++i, ++current_seed_i) {
        if (current_seed_i >= numer_calculated_seeds && current_seed_i < end) {
#if 0
					if ((offset + i)
          ...
          int query_representation_distance =
              distance_calculation_seed_list.GetDistances()[offset + i];
          if (distance_threshold >= query_representation_distance) {
            ungapped_extension_seed_list->AddSeed(query_seed_offset + i,
                                                  representative_position);
#if 0
						if (x_db_position == representative_position) {
      ...
      if (completed_hash_position_data_list_flag) {
        const vector<SeedSearcherGpu::QueryParameters::HashPositionData>
            &no_check_query_position_list =
                hash_position_data_list.no_check_position_data;
        size_t query_seed_offset =
      ...
      if ((!passing_distance_calculation_i_list.empty() ||
          ...
          !clustering_sequences_index.GetMembers(cluster_id, &members,
                                                 &members_length)) {
        size_t query_seed_offset = query_seed_list_offset;
        for (size_t pass_distance_calculation_i_list_i = 0;
             ...
             ++pass_distance_calculation_i_list_i) {
          ungapped_extension_seed_list->AddSeed(
              query_seed_offset + passing_distance_calculation_i_list
                                      [pass_distance_calculation_i_list_i],
        ...
        if (completed_hash_position_data_list_flag) {
          ...
          for (size_t no_check_query_position_data_i = 0;
               ...
               ++no_check_query_position_data_i) {
            ungapped_extension_seed_list->AddSeed(
                query_seed_offset + no_check_query_position_data_i, members,
                members_length);
  ...
  if (!sequences_index.GetValues(hash_position_data_list.hash, &values,
...
#if DEBUG
    hash_hits_count +=
        values_length * hash_position_data_list.check_position_data.size();
#endif
    const vector<SeedSearcherGpu::QueryParameters::HashPositionData> &
        check_query_position_list = hash_position_data_list.check_position_data;
    size_t query_seed_offset = query_seed_list_offset;
    ...
    const vector<SeedSearcherGpu::QueryParameters::HashPositionData>
        &no_check_query_position_list =
            hash_position_data_list.no_check_position_data;
    query_seed_offset =
        query_seed_list_offset + check_query_position_list.size();
    for (size_t no_check_query_position_data_i = 0;
         no_check_query_position_data_i < no_check_query_position_list.size();
         ++no_check_query_position_data_i) {
      ungapped_extension_seed_list->AddSeed(
