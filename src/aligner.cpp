...
void Aligner::BuildDatabaseParameters(
  ...
  database_parameters.seed_search_parameters_build_parameters
      .sequence_delimiter = database_parameters.sequence_delimiter;
  database_parameters.seed_search_parameters_build_parameters.clustering =
      parameters.clustering;
  database_parameters.seed_search_parameters_build_parameters
      .subsequence_length = parameters.clustering_subsequence_length;
  AlphabetCoder coder(*parameters.sequence_type_ptr);
  if (parameters.hash_alphabet_sets.empty()) {
    database_parameters.seed_search_parameters_build_parameters
        .max_indexing_code = coder.GetMaxRegularLetterCode();
  } else {
    ...
    for (AlphabetCoder::Code code = coder.GetMinCode(); code <= max_code;
      ...
      AlphabetCoder::Code reduced_code = hash_reduced_alphabet_coder.Encode(c);
    ...
    ReducedAlphabetCoder similarity_reduced_alphabet_coder(
        *(parameters.sequence_type_ptr), parameters.similarity_alphabet_sets);
...
void Aligner::Presearch(
  ...
  vector<AlignerPresearchThread::TempChainFilterSeedList>
      temp_chain_filter_seed_lists(parameters.number_threads);
  vector<vector<SeedSearcherGpu::Hit> > chain_filter_seed_lists(
      queries.GetNumberOfSequences());
  vector<vector<std::vector<AlignmentPosition> > >
      alignment_start_positions_in_database_list(parameters.number_threads);
...
void Aligner::BuildResults(
  ...
  for (uint32_t query_id = 0; query_id < queries.GetNumberOfSequences();
    ...
    max_result = std::max(max_result, presearch_results_list[query_id].size());
    for (uint32_t result_id = 0;
         result_id < presearch_results_list[query_id].size(); ++result_id) {
      result_ids_list[presearch_results_list[query_id][result_id]
                          .database_chunk_id]
          .push_back(make_pair(query_id, result_id));
    }
  }
  ...
  for (size_t i = 0; i < parameters.number_threads; ++i) {
    ...
    threads.create_thread(
        boost::bind(&AlignerBuildResultsThread<DatabaseType>::Run, t, p));
  }
  if (database_preload) {
    AlignerBuildResultsThread<DatabaseType> t;
    AlignerBuildResultsThread<DatabaseType>::Parameters p;
