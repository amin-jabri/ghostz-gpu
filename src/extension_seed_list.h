...
class ExtensionSeedList {
 public:
  ...
  void StartRun() { state_ = kRun; }
  void FinishRun() { state_ = kEnd; }

  void SetIdle() { state_ = kIdle; }

  void SetUp(size_t threhold_size, HostSeedsMemory *host_memory);
  HostSeedsMemory *GetSeedsMemory() { return seeds_memory_; }
  ...
  size_t GetForwardGpuSeedsOffset() { return forward_offset_; }

  size_t GetReverseGpuSeedsOffset() { return reverse_offset_; }

  size_t GetForwardGpuSeedsLength() { return forward_length_; }
  size_t GetReverseGpuSeedsLength() { return reverse_length_; }

  size_t GetSeedsLength() { return seed_data_list_.size(); }
