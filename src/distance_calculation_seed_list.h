...
class DistanceCalculationSeedList {
 public:
  ...
  size_t GetNumberCalculatedSeeds() { return number_calculated_seeds_; }
  size_t GetNumberHashPositionDataListIds();
  size_t GetNumberSeeds() { return size_; }

  size_t GetNumberSeedsThreshold() { return threhold_size_; }
  ...
  size_t GetDistancesLength() { return distnaces_.size(); }
#if 0
...
inline void DistanceCalculationSeedList::AddSeed(
  ...
  if (size_ < threhold_size_) {
    ...
    over_query_concatenated_positions_.push_back(query_concatenated_position);
...
inline DistanceCalculatorGpu::Distance*
DistanceCalculationSeedList::GetDistances() {
  return &distnaces_[0];
}
