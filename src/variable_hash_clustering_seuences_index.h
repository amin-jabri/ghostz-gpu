...
class VariableHashClusteringSeuencesIndex {
 public:
  ...
  int Build(std::vector<std::pair<HashKey, RepresentativeData> >::iterator
                hash_representation_pairs_first,
            std::vector<std::pair<HashKey, RepresentativeData> >::iterator
                hash_representation_pairs_last,
            std::vector<std::pair<ClusterId, Position> >::iterator
                cluster_id_member_pairs_first,
            std::vector<std::pair<ClusterId, Position> >::iterator
                cluster_id_pairs_last,
            const HashFunction &hash_function);

  int GetHashKey(const AlphabetCoder::Code *sequence, HashKey *hash_key,
                 uint32_t *hash_length) const;

  int GetHashKey(const AlphabetCoder::Code *sequence, HashKey *hash_key) const;
 ...
 private:
  ...
  typedef DenseHashInvertedIndex<HashFunction::Hash, RepresentativeData>
...
inline int VariableHashClusteringSeuencesIndex::GetMembers(
    ...
    size_t *members_length) const {
  return members_index_.GetValues(cluster_id, members, members_length);
}
