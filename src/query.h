...
class Query {
 public:
  ...
  virtual AlphabetCoder::Code *GetSequence() { return &sequence_[0]; }

  virtual uint32_t GetSequenceLength() { return sequence_.size(); }

  virtual uint32_t GetRealSequenceLength() { return sequence_.size(); }

  virtual uint32_t GetRealStart(uint32_t pos) { return pos; }

  virtual uint32_t GetRealEnd(uint32_t pos) { return pos; }

  virtual uint32_t GetDistanceFromStartDelimiter(uint32_t pos) { return pos; }
