...
SeedSearcherDatabaseParameters::SeedSearcherDatabaseParameters()
    : clustering_(false),
      ...
      cluster_threshold_rate_(0.0f),
      sequence_(NULL),
      redueced_code_map_(0) {}
...
int SeedSearcherDatabaseParameters::Build(AlphabetCoder::Code *sequence,
  ...
  for (uint32_t i = 0; i < number_letters; ++i) {
    if (i < parameters.hash_code_map.size()) {
      ...
      if (c <= parameters.max_indexing_code) {
        code_scores[c] = std::max(
            code_scores[c], score_matrix.GetMatrix()[i * number_letters + i]);
      }
    }
  }
...
#endif
  hash_function_ = SequencesIndex::HashFunction(
      parameters.max_indexing_code, score_threshold_, max_length,
      parameters.hash_code_map, code_scores);

  std::tr1::unordered_map<SequencesIndex::HashFunction::Hash, uint32_t>
      hash_map(0);
  std::tr1::unordered_map<SequencesIndex::HashFunction::Hash,
                          uint32_t>::iterator find_it;
  vector<pair<SequencesIndex::HashKey, SequencesIndex::Value> >
      indexed_key_value_pairs(0);
  ...
  for (uint32_t sequence_i = 0; sequence_i < length_; ++sequence_i) {
    ...
    int ret =
        hash_function_.CalculateHash(sequence_ + sequence_i, &h, &hash_length);
    if (!ret) {
      ...
      uint32_t seed_position =
          SeedSearcherCommon::CalculateSeedPosition(sequence_i, hash_length);
      if ((prev_hash_length != 0) && ((prev_hash_length - 1) >= hash_length)) {
...
// omp_set_num_threads(parameters.number_threads);
    ...
    for (uint32_t hashs_i = 0; hashs_i < hashs_size; ++hashs_i) {
      ...
      if (!sequences_index_.GetValues(hash, &values, &values_length)) {
        for (size_t subsequences_i = 0; subsequences_i < values_length;
             ++subsequences_i) {
          SequencesIndex::Value subsequence_center = values[subsequences_i];
          if (subsequence_center < one_direction_length) {
            clustering_data[subsequence_center].clustering_state =
                kNonClustering;
          } else {
            AlphabetCoder::Code *subsequence_sequence =
                sequence_ + subsequence_center - one_direction_length;
            for (size_t subsequence_i = 0; subsequence_i < subsequence_length_;
                 ++subsequence_i) {
              if (subsequence_sequence[subsequence_i] == sequence_delimiter_) {
                clustering_data[subsequence_center].clustering_state =
                    kNonClustering;
                break;
              }
        ...
        for (size_t subsequences_i = 0; subsequences_i < values_length;
             ++subsequences_i) {
          SequencesIndex::Value representative_center = values[subsequences_i];
          ...
          for (size_t member_i = subsequences_i + 1; member_i < values_length;
               ++member_i) {
            ...
            int mismatch_count = SeedSearcherCommon::CalculateDistance(
                ...
                non_clustering_member_sequence_center, subsequence_length_,
                redueced_code_map_);
            if (mismatch_count <= max_mismatch_count) {
              non_clustering_member_data.clustering_state = kMember;
...
#if DEBUG
        AlphabetCoder::Code *representative_sequence =
            sequence_ + clustering_data[i].representative_center;
    ...
    vector<pair<ClusteringSequencesIndex::HashKey,
                ClusteringSequencesIndex::RepresentativeData> >
        indexed_hash_representations_pairs;
    vector<pair<ClusteringSequencesIndex::ClusterId,
                ClusteringSequencesIndex::Position> >
        indexed_cluster_id_position_pairs;
    std::tr1::unordered_map<ClusteringSequencesIndex::Position,
                            ClusteringSequencesIndex::ClusterId>
        cluster_id_map;
        ...
        for (size_t values_i = 0; values_i < values_length; ++values_i) {
          ...
          if (clustering_data[sequence_center].clustering_state ==
            ...
            indexed_hash_representations_pairs.push_back(
                make_pair(hash, make_pair(number_clusters, sequence_center)));
            cluster_id_map.insert(make_pair(sequence_center, number_clusters));
            ++number_clusters;
            ...
            indexed_key_value_pairs.push_back(make_pair(hash, sequence_center));
          }
        }
      }
    }
    ...
    cout << "total number of subsequences is           "
         ...
         << endl;
    cout << "representative_count is                   " << representative_count
         << endl;
...
int SeedSearcherDatabaseParameters::BuildCommonParameters(
    std::istream &is, CommonParameters *common_parameters) {
  ...
  is.read((char *)&common_parameters->redueced_code_map[0],
          sizeof(common_parameters->redueced_code_map[0]) *
              common_parameters->redueced_code_map.size());
...
int SeedSearcherDatabaseParameters::Save(std::ostream &os) {
  ...
  os.write((char *)&cluster_threshold_rate_, sizeof(cluster_threshold_rate_));
...
bool SeedSearcherDatabaseParameters::IsClustering() { return clustering_; }

uint32_t SeedSearcherDatabaseParameters::GetSequenceLength() { return length_; }
...
SeedSearcherDatabaseParameters::SequencesIndex &
SeedSearcherDatabaseParameters::GetSequencesIndex() {
...
std::vector<AlphabetCoder::Code>
    &SeedSearcherDatabaseParameters::GetReduecedCodeMap() {
  return redueced_code_map_;
}

SeedSearcherDatabaseParameters::ClusteringSequencesIndex &
SeedSearcherDatabaseParameters::GetClusteringSequencesIndex() {
