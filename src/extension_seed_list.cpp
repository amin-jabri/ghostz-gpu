...
int ExtensionSeedList::PushExtensionSeeds(
    uint32_t query_id, uint32_t query_offset,
  ...
  if (seed_data_list_.size() + hits.size() < threhold_size_) {
    ...
    for (size_t i = 0; i < hits.size(); ++i) {
      ...
      size_t reverse_p =
          GetReverseGpuSeedsOffset() + GetReverseGpuSeedsLength();
      ...
      ++reverse_length_;
      size_t foward_p = GetForwardGpuSeedsOffset() + GetForwardGpuSeedsLength();
...
void ExtensionSeedList::ConvertGappedExtensionSeeds(
    Queries &queries, std::vector<uint32_t> &query_offsets) {
  ...
  for (size_t seed_i = 0; seed_i < number_seeds; ++seed_i) {
    ...
    temp_lengths_[reverse_p] = query->GetDistanceFromStartDelimiter(
        seeds_memory_query_concatenated_positions_[seed_data.reverse_seed_id] -
        query_offset);
    temp_lengths_[foward_p] = query->GetDistanceFromEndDelimiter(
  ...
  size_t reverse_end = GetReverseGpuSeedsOffset() + GetReverseGpuSeedsLength();
