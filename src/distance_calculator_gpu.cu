...
__device__ DistanceCalculatorGpu::Distance CalculateDisntace(
  ...
  for (uint32_t i = 0; i < length; ++i) {
    const uint32_t s0_c =
        packed_alphabet_code::GetAlphabetCode<cuda_common::kFoward>(
...
#endif
    distance +=
        (__ldg(&redueced_code_map[s0_c]) != __ldg(&redueced_code_map[s1_c]))
            ? 1
            : 0;
  }
  return distance;
...
#endif
    distance += (redueced_code_map[s0_c] != redueced_code_map[s1_c]) ? 1 : 0;
  }
  return distance;
}
...
__device__ DistanceCalculatorGpu::Distance CalculateDisntaceDevice(
  ...
  DistanceCalculatorGpu::Distance distance = 0;
  uint32_t offset = 0;
  for (; offset + packed_alphabet_code::kNumberOfCodesInBlock <=
         subsequence_length;
...
#endif
    distance +=
        CalculateDisntace(s0_code_block, s1_code_block, redueced_code_map);
  }
...
__global__ void CalculateDisntacesKernel(
  ...
  assert(subsequence_length + packed_alphabet_code::kNumberOfCodesInBlock <=
         cuda_common::kMaxLoadLength *
             packed_alphabet_code::kNumberOfCodesInBlock);
  ...
  int number_group_works =
      (number_distances + number_members - 1) / number_members;
  ...
  for (int group_work_i = group_idx; group_work_i < group_work_end;
    ...
    group_loader.Load(
        ...
        min((int)number_distances - group_extension_begin, number_members),
        s_group_load_temp, s0_cache);
#if 0
		// debug /////////////////////////////////////
    ...
    if (thread_work_id < number_distances) {
      distances[extension_id] = CalculateDisntaceDevice(
          s0_cache, sequence0_position, s1_cache, sequence1_position,
...
int DistanceCalculatorGpu::CalculateDistancesAsync(
    size_t size, uint32_t* query_concatenated_centers,
    uint32_t* database_centers, Distance* distances,
    uint32_t* d_query_concatenated_centers, uint32_t* d_database_centers,
    Distance* d_distances, cudaStream_t& stream) {
  CUDA_CHECK_RETURN(cudaMemcpyAsync(
      d_query_concatenated_centers, query_concatenated_centers,
      sizeof(query_concatenated_centers[0]) * size, cudaMemcpyDefault, stream));

  CUDA_CHECK_RETURN(cudaMemcpyAsync(d_database_centers, database_centers,
                                    sizeof(database_centers[0]) * size,
  ...
  size_t shared_memory_size =
      ...
      GroupLoader<const packed_alphabet_code::PackedAlphabetCode*,
                  distance_calculator_gpu_kernel::kLoadLength>::
          GetTotalSharedMemorySize(
              distance_calculator_gpu_kernel::kNumberThreads);
  distance_calculator_gpu_kernel::
      CalculateDisntacesKernel<<<distance_calculator_gpu_kernel::kNumberBlocks,
                                 distance_calculator_gpu_kernel::kNumberThreads,
                                 shared_memory_size, stream>>>(
          d_concatenated_query_sequence_, d_database_sequence_, size,
          d_query_concatenated_centers, d_database_centers,
          d_redueced_code_map_, subsequence_length_, d_distances);

  CUDA_CHECK_RETURN(cudaMemcpyAsync(distances, d_distances,
                                    sizeof(distances[0]) * size,
                                    cudaMemcpyDefault, stream));
