...
GappedExtender::GappedExtender() : dp_cells_(0), gap_states_(0) {}
...
bool GappedExtender::ExtendOneSide(
  ...
  *best_score = 0;
  if (edit_blocks == NULL) {
    return ExtendOneSideScoreOnly(
        sequence0, sequence0_length, sequence1, sequence_delimiter, reversed,
        score_matrix, gap_open, gap_extention, cutoff, best_score,
  ...
  *best_sequence1_position = -increment;
  ...
  for (sequence1_position = 0;
    ...
    if (gap_extention > 0) {
      state_index =
          GetGapStateIndex(array_end - array_start + number_extra_cells);
    } else {
    ...
    edit_script[edit_script_index] =
        &(gap_states_[state_index]
              .state_array[gap_states_[state_index].used + 1]);
    ;
    edit_script_offset[edit_script_index] = array_start;
    edit_script_array =
        edit_script[edit_script_index] - edit_script_offset[edit_script_index];

    const int *matrix_row =
        score_matrix_ptr + sequence1[sequence1_position] * number_letters;
    ...
    gap_states_[state_index].used +=
        array_end - edit_script_offset[edit_script_index];
  }
    ...
    for (array_index = array_start + 1; array_index < array_end;
      ...
      if (max_score - score > cutoff) {
        ...
        score_array[array_index].best_gap =
            max(score - gap_init, score_gap_column - gap_extention);
        score_gap_row = max(score - gap_init, score_gap_row - gap_extention);
        score_array[array_index].best = score;
      }
    }
...
void GappedExtender::PickOptimalPath(const vector<uint8_t *> &edit_script,
                                     const vector<uint32_t> &edit_script_offset,
                                     int sequence0_position,
                                     int sequence1_position, int increment,
  ...
  while (sequence1_position * increment >= 0 &&
    ...
    uint32_t edit_script_index = sequence1_position * increment;
    next_op = edit_script[edit_script_index]
                         [(sequence0_position * increment) -
