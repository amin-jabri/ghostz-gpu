...
sequence0_multi_code_cache,
                packed_alphabet_code::PackedAlphabetCode
sequence1_multi_code_cache,
                const AlphabetCoder::Code sequence_delimiter, bool reversed,
                const int*  __restrict__  score_matrix,
                const uint32_t number_letters, const int cutoff, const int
trigger,
        ...
        uint32_t sift_offset =
                        reversed ? 8 *
(packed_alphabet_code::kNumberCodesInBlock - 1) : 0;

        for (uint32_t i = 0; i < packed_alphabet_code::kNumberCodesInBlock; ++i)
{
                const uint32_t shift_size = (8 * i);
                const uint32_t shift = reversed ? sift_offset - shift_size :
shift_size;
                const AlphabetCoder::Code s0_c = (sequence0_multi_code_cache >>
shift)
                                & mask;
                const AlphabetCoder::Code s1_c = (sequence1_multi_code_cache >>
shift)
                                & mask;

                stop_flag = stop_flag || s0_c == sequence_delimiter
                                || s1_c == sequence_delimiter;
                score += stop_flag ? 0 : score_matrix[s0_c * number_letters +
s1_c];
                best_score = score > best_score ? score : best_score;
                threshold = best_score - cutoff;
                stop_flag = stop_flag || score <= threshold || best_score >
trigger;
                if (stop_flag) {
                        break;
                }
...
sequence0_multi_code,
                const uint32_t s0_start_p,
                const packed_alphabet_code::PackedAlphabetCode*
sequence1_multi_code,
                const uint32_t s1_start_p, const AlphabetCoder::Code
sequence_delimiter,
...
s1_cache[0]);
                ...
                if (UngappedExtendOneSideCacheRef(s0_cache[0], s1_cache[0],
                                sequence_delimiter, reversed, score_matrix,
number_letters,
...
sequence1_multi_code,
                ...
                const int* __restrict__ score_matrix,
                const uint32_t number_letters, const int* cutoffs, const int*
triggers,
                int* best_scores) {
        ...
        for (uint32_t i = thread_id; i < number_extensions; i += thread_id_skip)
{
                ...
                current_best_score = UngappedExtendOneSideDeviceRef(
                                sequence0_multi_code, s0_p - 1,
sequence1_multi_code, s1_p - 1,
                                sequence_delimiter, true, score_matrix,
number_letters, cutoff,
                                trigger, current_best_score);

                if (current_best_score <= trigger) {
                        current_best_score = UngappedExtendOneSideDeviceRef(
                                        sequence0_multi_code, s0_p,
sequence1_multi_code, s1_p,
                                        sequence_delimiter, false, score_matrix,
number_letters,
