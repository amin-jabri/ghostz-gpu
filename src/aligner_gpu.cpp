...
void AlignerGpu::Presearch(
  ...
  vector<AlignerGpuPresearchThread::TempChainFilterSeedList>
  ...
  queries_concatenated_sequence.resize(total_length_query_sequences -
                                       queries.GetNumberOfSequences() + 1);
  ...
  size_t gpu_stream_controller_buffer_size =
      ...
      ((number_presearch_threads + gpu_ids.size() - 1) / gpu_ids.size());
  for (size_t i = 0; i < number_presearch_threads; ++i) {
    ...
    threads.create_thread(boost::bind(&AlignerGpuPresearchThread::Run, t, p));
  }
  if (database_preload) {
    AlignerGpuPresearchThread t;
    AlignerGpuPresearchThread::ThreadParameters p;
...
void AlignerGpu::SetQueriesData(
    Queries &queries, AlphabetCoder::Code sequence_delimiter,
    AligningParameters &parameters,
    std::vector<AlphabetCoder::Code> &queries_concatenated_sequence,
    std::vector<uint32_t> &query_sequence_offsets,
