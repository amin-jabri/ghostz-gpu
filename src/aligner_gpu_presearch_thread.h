...
class AlignerGpuPresearchThread {
 ...
 private:
  ...
  int PopHashPositionDataListIdsForRepresentativeSearch(
      std::vector<uint32_t> *query_ids);
  int PopHashPositionDataListIdsForSeedSearch(std::vector<uint32_t> *query_ids);
  ...
  int RunUngappedExtension(size_t seed_lists_id,
                           ...
                           HostSeedsMemory &host_seeds_memory);
  int RunGappedExtension(size_t seed_lists_id, GpuStreamController &controller,
                         HostSeedsMemory &host_seeds_memory);
  int MoveDistanceCalculationResults(
      int gpu_controller_resource_id, GpuStreamController &controller,
      DistanceCalculationSeedList *distance_calculation_seed_list);
  int MoveUngappedExtensionWithTriggerResults(
      int gpu_controller_resource_id, GpuStreamController &controller,
  ...
  int ExtendOnCpu(UngappedExtender &ungapped_extender,
                  GappedExtender &gapped_extender, uint32_t query_id,
                  Query *query,
  ...
  int UngappedExtendBackSeedsOnCpu(
      size_t seed_list_id, size_t extension_size,
  ...
  int GappedExtendBackSeedsOnCpu(
      size_t seed_list_id, size_t extension_size,
  ...
  std::vector<std::pair<uint32_t, uint32_t> >
      temp_ungapped_extension_seed_data_;
  ...
  GpuStreamController::DistanceCalculationSeedsParameters
      last_distance_calculation_seeds_parameters_list_[kNumberOfGpuResources];
  GpuStreamController::UngappedExtensionWithTriggerSeedsParameters
