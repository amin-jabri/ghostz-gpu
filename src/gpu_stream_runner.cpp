...
GpuStreamRunner::GpuStreamRunner()
    : distance_calculator_(NULL),
      ungapped_extender_(NULL),
      gapped_extender_(NULL) {
...
int GpuStreamRunner::RunDistanceCalculationAsync(
    HostSeedsMemory &host_buffer, DeviceSeedsMemory &device_buffer,
    size_t size) {
  distance_calculator_->CalculateDistancesAsync(
      size, host_buffer.GetQueryConcatenatedPositions(),
...
int GpuStreamRunner::RunUngappedExtensionWithTriggerAsync(
  ...
  ungapped_extender_->SetQuerySeeds(
      query_seed_size, size, host_buffer.GetQuerySeedIds(),
      host_buffer.GetQuerySeedStarts(), device_buffer.GetTempStorageSize(),
      device_buffer.GetTempStorage(), device_buffer.GetQuerySeedIds(),
      device_buffer.GetQuerySeedStarts(), device_buffer.GetQueryIds(),
      device_buffer.GetQueryConcatenatedPositions(),
      device_buffer.GetDatabasePositions(), stream_[0]);
  ungapped_extender_->ExtendWithTriggerAsync(
      size, NULL, NULL, host_buffer.GetDatabasePositions(),
...
int GpuStreamRunner::RunGappedExtensionAsync(HostSeedsMemory &host_buffer,
                                             DeviceSeedsMemory &device_buffer,
                                             size_t foward_offset,
                                             size_t foward_size,
                                             size_t reverse_offset,
                                             size_t reverse_size) {
  gapped_extender_->ConvertToGappedExtensionSeedsAsync(
      reverse_size, true, host_buffer.GetQuerySeedIds() + reverse_offset,
  ...
  gapped_extender_->ExtendOneSideScoreOnlyAsync(
      reverse_size, true,
      host_buffer.GetQueryConcatenatedPositions() + reverse_offset,
      host_buffer.GetDatabasePositions() + reverse_offset,
  ...
  gapped_extender_->ConvertToGappedExtensionSeedsAsync(
      foward_size, false, host_buffer.GetQuerySeedIds() + foward_offset,
