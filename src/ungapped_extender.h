...
class UngappedExtender {
 public:
  static bool ExtendOneSideScoreOnly(
      ...
      int *best_score);  //__attribute__((noinline));

  static bool ExtendOneSide(
      const AlphabetCoder::Code *sequence0,
      const AlphabetCoder::Code *sequence1,
      const AlphabetCoder::Code sequence_delimiter, bool reversed,
...
inline bool UngappedExtender::ExtendOneSideScoreOnly(
    const AlphabetCoder::Code *sequence0, const AlphabetCoder::Code *sequence1,
    const AlphabetCoder::Code sequence_delimiter, bool reversed,
    const ScoreMatrix &score_matrix, int cutoff, int *best_score) {
  ...
  if (sequence1[position] != sequence_delimiter &&
    ...
    int score = 0;
    do {
      // std::cout << position << " " <<(int)sequence0[position] << " " <<
      // (int)sequence1[position] << " "  <<  score << std::endl;
      if (sequence1[position] == sequence_delimiter ||
          sequence0[position] == sequence_delimiter) {
        break;
      }
      score += sm[sequence1[position] * number_letters + sequence0[position]];
      position += increment;
      if (score > 0) {
        do {
          b_score += score;
          ...
          score =
              sm[sequence1[position] * number_letters + sequence0[position]];
          position += increment;
        } while (score > 0);
      }
