...
AlignerGpuData::AlignerGpuData() : gpu_id_(0) {}

AlignerGpuData::AlignerGpuData(int gpu_id) : gpu_id_(gpu_id) {
  SetGpuId(gpu_id);
}
...
int AlignerGpuData::GetGpuId() { return gpu_id_; }
...
packed_alphabet_code::PackedAlphabetCode*
AlignerGpuData::GetGpuDatabaseSequence() {
...
packed_alphabet_code::PackedAlphabetCode*
AlignerGpuData::GetGpuQueriesSequence() {
...
void AlignerGpuData::SetPackedAlphabetCodeSequenceToGpu(
    size_t length, const AlphabetCoder::Code* sequence,
    thrust::device_vector<packed_alphabet_code::PackedAlphabetCode>&
        d_packed_alphabet_code_sequence) {
  ...
  packed_alphabet_code::PackAlphabetCodeSequence(
      sequence, length, &temp_packed_alphabet_code_sequence_[0]);
  size_t new_d_packed_alphabet_code_sequence_size =
      packed_sequence_length + (2 * cuda_common::kMaxLoadLength);
  if (d_packed_alphabet_code_sequence.size() <
      new_d_packed_alphabet_code_sequence_size) {
    d_packed_alphabet_code_sequence.resize(
        new_d_packed_alphabet_code_sequence_size);
  }
  thrust::copy(
      temp_packed_alphabet_code_sequence_.begin(),
      temp_packed_alphabet_code_sequence_.begin() + packed_sequence_length,
      d_packed_alphabet_code_sequence.begin() + cuda_common::kMaxLoadLength);
}
