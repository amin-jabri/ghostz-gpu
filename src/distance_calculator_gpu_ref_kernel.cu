...
:
                ...
                distance += CalculateDisntaceDeviceCach(s0_cache[0],
s1_cache[0],
...
sequence0_multi_code,
                const packed_alphabet_code::PackedAlphabetCode*
sequence1_multi_code,
                const uint32_t number_distances, const uint32_t *
sequence0_positions,
...
foward_direction;
                const uint32_t s1_p = sequence1_positions[i]
                                + cuda_common::kOneSideMarginSize -
foward_direction;
                distances[i] = CalculateDisntaceDevice(sequence0_multi_code,
s0_p,
