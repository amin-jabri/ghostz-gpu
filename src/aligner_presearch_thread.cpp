...
AlignerPresearchThread::AlignerPresearchThread() : thread_parameters_(NULL) {}
...
void AlignerPresearchThread::Run(ThreadParameters &thread_parameters) {
  ...
  if (thread_id < 0) {
    ...
    SeedSearcher::QueryParameters::BuildParameters
    ...
    seed_search_query_parameters_build_parameter
        .ungapped_extension_cutoffs_ptr =
  ...
  while (shared_parameters->database->GetChunkId() <
    ...
    if (thread_id < 0) {
      ...
      while (!PopHashPositionDataListIds(&hash_position_data_list_ids)) {
        while (CanRunSeedSearch()) {
          ...
          shared_parameters->seed_searcher->Search(
              hash_position_data_list_id, hitting_query_position_data_i_list,
      ...
      while (!PopQueryIdsForChainFilter(&chain_filter_query_ids)) {
        size_t next_chain_filter_query_id = chain_filter_query_ids.back();
        chain_filter_query_ids.pop_back();
        Query *extended_query = queries.GetQuery(next_chain_filter_query_id);
...
#endif
        chain_filter.Filter(
            extended_query->GetSequence(), extended_query->GetSequenceLength(),
            (*shared_parameters
                  ->ungapped_extension_cutoffs)[next_chain_filter_query_id],
            shared_parameters->database_concatenated_sequence, *next_hits);

        tmp_cpu_presearch_results.clear();
        Extend(ungapped_extender, gapped_extender, next_chain_filter_query_id,
               extended_query,
               shared_parameters->database_concatenated_sequence, parameters,
               (*shared_parameters->ungapped_extension_cutoffs),
...
void AlignerPresearchThread::AddResults(
...
int AlignerPresearchThread::Extend(
...
void AlignerPresearchThread::SetChainFilteringSeeds() {
  ...
  int thread_id = thread_parameters_->thread_id;
  std::vector<AlignerPresearchThread::TempChainFilterSeedList>
