...
class SeedSearcherGpuQueryParameters {
 public:
  ...
  uint32_t *GetSeedQueryIds() { return &seed_query_ids_[0]; }

  uint32_t *GetSeedQueryPositions() { return &seed_query_positions_[0]; }
 ...
 private:
  ...
  typedef std::vector<TempHashPositionDataBucket>
      TempHashPositionDataBucketList;
  ...
  std::vector<TempHashPositionDataBucketList>
      temp_hash_position_data_bucket_lists_;
