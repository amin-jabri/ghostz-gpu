...
int ReducedAlphabetVariableHashFunction::CalculateHash(
  ...
  for (ret_hash_length = 0; ret_hash_length < max_length_; ++ret_hash_length) {
    if (sequence[ret_hash_length] < reduced_code_map_.size() &&
        reduced_code_map_[sequence[ret_hash_length]] <= max_code_) {
      if (ret_hash_length >= kMinHashLength && score >= score_threshold_) {
        break;
      }
...
ReducedAlphabetVariableHashFunction::Hash
ReducedAlphabetVariableHashFunction::GetMaxHash() const {
  return max_hash_;
}
...
int ReducedAlphabetVariableHashFunction::SetMaxHash(
    AlphabetCoder::Code max_code, uint32_t max_length, uint32_t shift_size) {
  max_hash_ = 0;
