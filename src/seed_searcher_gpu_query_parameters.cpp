...
int SeedSearcherGpuQueryParameters::Build(int thread_id, int number_threads,
          ...
          for (uint32_t query_sequence_i = subsequence_offset;
            ...
            if (!hash_function.CalculateHash(query_sequence + query_sequence_i,
              ...
              if (similarity_check_start <= seed_position &&
                ...
                temp_hash_position_data.similarity_check = false;
              }
    ...
    for (size_t data_i = 0; data_i < bucket.size(); ++data_i) {
      ...
      if (find_it == hash_map.end()) {
        ...
        temp_hash_position_data_lists[hash_position_data_list_id]
            .check_position_data.clear();
        temp_hash_position_data_lists[hash_position_data_list_id]
            .no_check_position_data.clear();
      } else {
      ...
      if (data.similarity_check) {
        temp_hash_position_data_lists[hash_position_data_list_id]
            .check_position_data.push_back(data.data);
      } else {
        temp_hash_position_data_lists[hash_position_data_list_id]
            .no_check_position_data.push_back(data.data);
      }
  ...
  for (uint32_t i = 0; i < temp_hash_position_data_lists.size(); ++i) {
    ...
    const HashPositionDataList &list =
        hash_position_data_lists_[list_offset + i];
    for (size_t j = 0; j < list.check_position_data.size();
         ++j, ++seed_offset) {
      seed_query_ids_[seed_offset] = list.check_position_data[j].query_id;
      seed_query_positions_[seed_offset] = list.check_position_data[j].position;
    }
    for (size_t j = 0; j < list.no_check_position_data.size();
         ++j, ++seed_offset) {
      seed_query_ids_[seed_offset] = list.no_check_position_data[j].query_id;
    ...
    prefix_hash_position_data_offset +=
        list.check_position_data.size() + list.no_check_position_data.size();
  }
  barrier->wait();
  if (thread_id == 0) {
    hash_position_data_offsets_.resize(number_using_hash_position_data_list_ +
    ...
    for (uint32_t i = 0; i < number_using_hash_position_data_list_; ++i) {
      ...
      prefix_sum += hash_position_data_lists_[i].check_position_data.size() +
                    hash_position_data_lists_[i].no_check_position_data.size();
    }
    assert(hash_position_data_offsets_[number_using_hash_position_data_list_] ==
           prefix_sum);
    hash_position_data_offsets_[number_using_hash_position_data_list_] =
        prefix_sum;
  }
...
std::vector<SeedSearcherGpuQueryParameters::HashPositionDataList>
    &SeedSearcherGpuQueryParameters::GetHashPositionDataLists() {
...
int SeedSearcherGpuQueryParameters::PopQueryIds(
    size_t number_queries, boost::mutex *next_query_id_mutex,
    std::vector<uint32_t> *query_ids) {
  uint32_t start_id = 0;
  uint32_t end_id = 0;
