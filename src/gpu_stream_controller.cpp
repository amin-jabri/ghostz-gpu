...
int GpuStreamController::Run(
  ...
  int id = available_resource_ids_.front();
  available_resource_ids_.pop();
  runners_[id].RunUngappedExtensionWithTriggerAsync(
      host_memory, device_memory_[id], parameters.query_seed_size,
      parameters.size);
  return id;
}
...
int GpuStreamController::Run(GappedExtensionSeesParameters &parameters,
  ...
  int id = available_resource_ids_.front();
  available_resource_ids_.pop();
  runners_[id].RunGappedExtensionAsync(
      host_memory, device_memory_[id], parameters.forward_offset,
      parameters.forward_length, parameters.reverse_offset,
