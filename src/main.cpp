...
void PrintUsage() {
  ...
  logger->Log(std::string("GHOSTZ-GPU - homology search tool. version " +
                          common::kVersion));
  logger->Log(
      "\n"
      "Command and Options\n"
      ...
      "\n"
      "  ghostz_gpu db [-i dbFastaFile] [-o dbName] [-C clustering][-l "
      "chunkSize]\n"
      ...
      "\n"
      "  ghostz_gpu aln [-i queries] [-o output] [-d database] [-v "
      "maxNumAliSub]\n"
      "             [-b maxNumAliQue] [-h hitsSize] [-l queriesChunkSize] [-q "
      "queryType]\n"
      "             [-t databaseType] [-F filter] [-a numThreads] [-g "
      "numGPUs]\n"
      "\n"
      "  Options:\n"
      "  (Required)\n"
