...
TranslatedDnaQuery::TranslatedDnaQuery(
    const Sequence &sequence, AlphabetCoder::Code sequence_delimiter,
    Translator &translator, SequenceFilterInterface *filter,
...
void TranslatedDnaQuery::SetEncodedSequence(const Sequence &sequence,
  ...
  sequence_.resize(
      TranslatedDnaQuery::GetSequenceLength(dna.GetSequenceData().size()));
...
void TranslatedDnaQuery::SetStaticalParameters(
    ScoreMatrix &score_matrix,
    Statistics::KarlinParameters &ungapped_ideal_karlin_parameters) {
  ProteinType t;
  Statistics statistics(t);
  bool ret = statistics.CalculateUngappedKarlinParameters(
      &sequence_[0], sequence_.size(), score_matrix,
      &ungapped_karlin_parameters_);
  if (!ret) {
