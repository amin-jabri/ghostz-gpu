...
ReducedAlphabetKMerHashFunction::ReducedAlphabetKMerHashFunction()
    : can_calculate_hash_flag_(false),
      max_code_(0),
      k_mer_length_(0),
      shift_size_(0),
      max_hash_(0),
...
ReducedAlphabetKMerHashFunction::ReducedAlphabetKMerHashFunction(
    AlphabetCoder::Code max_code, uint32_t k_mer_length,
    std::vector<AlphabetCoder::Code> &reduced_code_map)
    : can_calculate_hash_flag_(false),
      max_code_(max_code),
      k_mer_length_(k_mer_length),
      shift_size_(0),
      max_hash_(0),
      reduced_code_map_(reduced_code_map) {
...
int ReducedAlphabetKMerHashFunction::CalculateHash(
    const AlphabetCoder::Code *sequence, Hash *hash) const {
  Hash ret_hash = 0;
  for (uint32_t i = 0; i < k_mer_length_; ++i) {
    if (sequence[i] < reduced_code_map_.size() &&
        reduced_code_map_[sequence[i]] <= max_code_) {
...
ReducedAlphabetKMerHashFunction::Hash
ReducedAlphabetKMerHashFunction::GetMaxHash() const {
  return max_hash_;
}
...
int ReducedAlphabetKMerHashFunction::SetShiftSize(
...
int ReducedAlphabetKMerHashFunction::SetMaxHash(AlphabetCoder::Code max_code,
                                                uint32_t k_mer_length,
                                                uint32_t shift_size) {
  max_hash_ = 0;
  if ((shift_size * k_mer_length > sizeof(Hash) * 8) ||
      ((shift_size * k_mer_length == sizeof(Hash) * 8) &&
       (max_code == (1 << shift_size)))) {
    return 1;
  }
