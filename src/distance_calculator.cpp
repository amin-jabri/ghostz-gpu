...
DistanceCalculator::DistanceCalculator()
    : concatenated_query_sequence_(NULL),
      database_sequence_(NULL),
      redueced_code_map_(NULL) {}
...
DistanceCalculator::Distance DistanceCalculator::CalculateDistance(
  ...
  const AlphabetCoder::Code *subsequence0 = concatenated_query_sequence_ +
                                            query_concatenated_center -
                                            foward_direction;
  const AlphabetCoder::Code *subsequence1 =
      database_sequence_ + database_center - foward_direction;
...
#endif
    mismatch_count += (redueced_code_map_[subsequence0[i]] !=
                       redueced_code_map_[subsequence1[i]])
                          ? 1
                          : 0;
  }
...
int DistanceCalculator::CalculateDistances(size_t size,
                                           uint32_t *query_concatenated_centers,
