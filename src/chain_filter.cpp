...
void ChainFilter::Filter(const AlphabetCoder::Code *query_sequence,
  ...
  for (vector<Chain>::iterator it = chains_.begin(); it < chains_.end(); ++it) {
    *it = init_chain;
  }
  ...
  for (vector<Hit>::iterator hits_it = hits.begin(); hits_it != hits.end();
    ...
    if (chains_[offset].diagonal != diagonal ||
        !Connected(query_sequence, database_sequence, chains_[offset].query_end,
                   *hits_it, cutoff)) {
...
bool ChainFilter::Connected(const AlphabetCoder::Code *query_sequence,
                            const AlphabetCoder::Code *database_sequence,
                            uint32_t chain_query_end, const Hit &hit,
                            int cutoff) {
