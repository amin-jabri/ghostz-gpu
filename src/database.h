...
class Database {
 public:
  ...
  typedef typename SeedSearcherParameters::BuildParameters
      SeedSearcherParametersBuildParameters;
  typedef typename SeedSearcherParameters::CommonParameters
      SeedSearcherParametersCommonParameters;
  ...
  int GetChunkId() { return chunk_id_; }

  int GetPreloadedChunkId() { return preload_chunk_id_; }
  ...
  uint32_t GetConcatenatedSequenceLength() {
    return chunk_list_[chunk_ids_[kUsedChunkId]]
        ->GetConcatenatedSequenceLength();
  }
  ...
  SeedSearcherParametersCommonParameters &GetSeedSearcherCommonParameters() {
    return chunk_list_[chunk_ids_[kUsedChunkId]]
        ->GetSeedSearcherCommonParameters();
  }
  ...
  AlphabetCoder::Code GetSequenceDelimiter() { return sequence_delimiter_; }
...
unsigned int Database<TSeedSearcher>::ReadSequences(
  ...
  while (reader_ret) {
    unsigned int new_sum_length =
        sum_length + next_sequence_.GetSequenceData().size();
...
bool Database<TSeedSearcher>::BuildNextChunk() {
  ...
  if (parameters_.chunk_build_option ==
    ...
    chunk_list_[chunk_ids_[kUsedChunkId]]->Build(
        sequences, coder, parameters_.sequence_delimiter,
...
int Database<TSeedSearcher>::Preload(int id, PreloadTarget target) {
  ...
  if (all_on_memory_flag_) {
    ...
    if (!chunk_list_[chunk_list_id]) {
      chunk_list_[chunk_list_id] = DatabaseChunkTypePtr(new DatabaseChunkType);
      std::stringstream prefix;
      prefix.str("");
      prefix << filename_prefix_ << "_" << id;
