...
class SeedSearcher {
 ...
 private:
  ...
  int UngappedExtend(
      ...
      const AlphabetCoder::Code *database_sequence, uint32_t database_position,
      AlphabetCoder::Code sequence_delimiter, const ScoreMatrix &score_matrix,
      int cutoff,
  ...
  bool CheckSimilarity(AlphabetCoder::Code *query_sequence,
                       ...
                       AlphabetCoder::Code *database_sequence,
...
inline int SeedSearcher::UngappedExtend(
    ...
    const AlphabetCoder::Code *database_sequence, uint32_t database_position,
    AlphabetCoder::Code sequence_delimiter, const ScoreMatrix &score_matrix,
    int cutoff, int gapped_extension_trigger) {
  ...
  if (sum_score <= gapped_extension_trigger) {
    UngappedExtender::ExtendOneSideScoreOnly(
        query_sequence + query_position, database_sequence + database_position,
        sequence_delimiter, false, score_matrix, cutoff, &score);
    sum_score += score;
  }
