...
class OneMismatchHashGeneratorTest : public ::testing::Test {
 protected:
  virtual void SetUp() {
    ...
    k_mer_length_ = 2;
    hash_function_ = ReducedAlphabetKMerHashFunction(max_code_, k_mer_length_,
                                                     reduced_code_map_);
  }
...
TEST_F(OneMismatchHashGeneratorTest, GenerateOneMismatchHash) {
  ...
  one_mismatch_hash_generator.GenerateOneMismatchHash(
      basic_hash, k_mer_length_, max_code_, hash_function_.GetBitShiftSize(),
      &onemismatch_hashs);
  ...
  for (size_t i = 0; i < onemismatch_hashs.size(); ++i) {
    ...
    for (uint32_t hash_i = 0; hash_i < k_mer_length_; ++hash_i,
                  tmp_basic_hash >>= hash_function_.GetBitShiftSize(),
                  generated_hash >>= hash_function_.GetBitShiftSize()) {
