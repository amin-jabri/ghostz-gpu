...
TEST(RedicedAlphabetCoderTest, Read) {
  ...
  for (AlphabetCoder::Code code1 = alphabet_coder.GetMinCode();
       code1 <= alphabet_coder.GetMaxCode(); ++code1) {
    ...
    for (AlphabetCoder::Code code2 = alphabet_coder.GetMinCode();
         code2 <= alphabet_coder.GetMaxCode(); ++code2) {
      ...
      if (set_id < alphabet_sets.size()) {
        ...
        if (count < alphabet_sets[set_id].size()) {
          EXPECT_EQ(reduced_alphabet_coder.Encode(c1),
                    reduced_alphabet_coder.Encode(c2));
        } else {
          EXPECT_NE(reduced_alphabet_coder.Encode(c1),
                    reduced_alphabet_coder.Encode(c2));
        }
      } else {
        if (c1 != c2) {
