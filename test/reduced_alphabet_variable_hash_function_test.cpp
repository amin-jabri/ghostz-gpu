  ...
  virtual void SetUp() {
    ...
    for (uint32_t i = 0; i < number_letters; ++i) {
      if (i < reduced_code_map_.size()) {
        ...
        if (c <= max_code_) {
          code_score_[c] = std::max(
              code_score_[c], score_matrix.GetMatrix()[i * number_letters + i]);
        }
      }
    }
    hash_function_ = ReducedAlphabetVariableHashFunction(
        max_code_, score_threshold_, max_length_, reduced_code_map_,
        code_score_);
  }
...
TEST_F(ReducedAlphabetVariableHashFunctionTest, CalculateHash) {
  ...
  int score = 0;
  for (uint32_t i = 0; (i < max_length_) && (score < score_threshold_) &&
                       k_mer_coded_string[i] < reduced_code_map_.size();
       ++i) {
...
TEST_F(ReducedAlphabetVariableHashFunctionTest, SaveAndLoad) {
  ...
  ReducedAlphabetVariableHashFunction::Hash expected_hash = 0;
  int ret =
      hash_function_.CalculateHash(&k_mer_coded_string[0], &expected_hash);
