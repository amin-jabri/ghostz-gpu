...
TEST_F(GappedExtenderGpuTest, ExtendShortSequencesForwardWithMinXDrop) {
  ...
  thrust::host_vector<uint32_t,
                      thrust::cuda::experimental::pinned_allocator<uint32_t> >
      ids(1, 0);
  thrust::host_vector<uint32_t,
                      thrust::cuda::experimental::pinned_allocator<uint32_t> >
      sequence0_positions(number_extensions);
  thrust::host_vector<uint32_t,
                      thrust::cuda::experimental::pinned_allocator<uint32_t> >
      sequence1_positions(number_extensions);
  thrust::host_vector<int, thrust::cuda::experimental::pinned_allocator<int> >
      scores(number_extensions, 0);
  ...
  e.ExtendOneSideScoreOnlyAsync(
      number_extensions, false, &sequence0_positions[0],
...
TEST_F(GappedExtenderGpuTest, ExtendShortSequencesReverse) {
  ...
  e.ExtendOneSideScoreOnlyAsync(
      number_extensions, true, &sequence0_positions[0], &sequence1_positions[0],
      &scores[0], thrust::raw_pointer_cast(d_sequence0_positions.data()),
      thrust::raw_pointer_cast(d_sequence1_positions.data()),
      thrust::raw_pointer_cast(d_scores.data()), stream);
