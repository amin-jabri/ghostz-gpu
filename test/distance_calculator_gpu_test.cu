...
TEST_F(DistanceCalculatorGpuTest, CalculateDistances) {
  ...
  gpu_data.SetGpuReducedCodeMap(&reduced_code_map_[0],
                                reduced_code_map_.size());
  ...
  thrust::device_vector<DistanceCalculatorGpu::Distance> d_distances(
      sequence0_positions.size());
  ...
  distance_calculator.CalculateDistancesAsync(
      3, &sequence0_positions[0], &sequence1_positions[0], &distances[0],
