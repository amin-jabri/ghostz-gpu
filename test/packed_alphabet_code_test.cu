...
TEST_F(PackedAlphabetCodeTest, GetPackedAlphabetCodeSequenceLength) {
  size_t length = 10;
  EXPECT_EQ((length + packed_alphabet_code::kNumberOfCodesInBlock - 1) /
...
TEST_F(PackedAlphabetCodeTest, GetAlphabetCodeFoward) {
  ...
  packed_alphabet_code::PackAlphabetCodeSequence(
      &encoded_seq0[0], encoded_seq0.size(), &packed_code_seq0[1]);
  ...
  packed_alphabet_code::PackedAlphabetCode block =
      packed_alphabet_code::GetPackedAlphabetCode<cuda_common::kFoward>(
          code_blocks, block_sift, next_block_sift, &next_bolock_position,
          &temp_code_block);
  EXPECT_EQ(
      0, packed_alphabet_code::GetAlphabetCode<cuda_common::kFoward>(block, 0));
  EXPECT_EQ(
      3, packed_alphabet_code::GetAlphabetCode<cuda_common::kFoward>(block, 3));
}
...
TEST_F(PackedAlphabetCodeTest, GetAlphabetCodeReverse) {
  ...
  EXPECT_EQ(3, packed_alphabet_code::GetAlphabetCode<cuda_common::kReverse>(
                   block, 0));
  EXPECT_EQ(0, packed_alphabet_code::GetAlphabetCode<cuda_common::kReverse>(
                   block, 3));
}
