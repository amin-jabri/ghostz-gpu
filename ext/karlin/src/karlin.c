...
double BlastKarlinLHtoK(double* sfp, int min, int max, double lambda, double H);
double BlastKarlinLambdaNR(double* sfp, int min, int max);
double BlastKarlinLtoH(double* sfp, int min, int max, double lambda);

void BlastKarlinBlkCalc(double* scoreProbabilities, int min, int max,
                        float* lambda, float* K, float* H) {
  float l;
  float k;
  float h;
  /* Calculate the parameter Lambda */
  l = BlastKarlinLambdaNR(scoreProbabilities, min, max);

  /* Calculate H */
  h = BlastKarlinLtoH(scoreProbabilities, min, max, l);

  /* Calculate K */
  k = BlastKarlinLHtoK(scoreProbabilities, min, max, l, h);
  *lambda = l;
  *H = h;
  *K = k;
}
...
int BlastScoreChk(int lo, int hi);

double BlastKarlinLHtoK(double* sfp, int min, int max, double lambda,
                        double H) {
...
*/
double BlastKarlinLambdaBis(double* sfp, int min, int max) {
  double* sprob;
  ...
  for (lambda = 0.;;) {
    ...
    if (x1 > 0.) {
      for (sum = 0., i = low; i <= high; ++i) sum += sprob[i] * (x1 *= x0);
    } else {
      for (sum = 0., i = low; i <= high; ++i) sum += sprob[i] * exp(up * i);
    }
    if (sum >= 1.0) break;
    lambda = up;
  }
  ...
  for (j = 0; j < BLAST_KARLIN_LAMBDA_ITER_DEFAULT; ++j) {
    ...
    if (x1 > 0.) {
      ...
      for (sum = 0., i = low; i <= high; ++i) sum += sprob[i] * exp(newval * i);
    }
    if (sum > 1.0)
      up = newval;
    else
...
double BlastKarlinLambdaNR(double* sfp, int min, int max) {
  ...
  for (j = 0; j < 20; ++j) { /* limit of 20 should never be close-approached */
    ...
    if (ABS(amt / lambda0) < BLAST_KARLIN_LAMBDA_ACCURACY_DEFAULT) {
      ...
      */
      if (lambda0 > BLAST_KARLIN_LAMBDA_ACCURACY_DEFAULT) return lambda0;
      break;
    }
  }
...
double BlastKarlinLtoH(double* sfp, int min, int max, double lambda) {
  ...
  if (BlastScoreChk(min, max) != 0) return -1.;
...
int BlastScoreChk(int lo, int hi) {
  if (lo >= 0 || hi <= 0 || lo < BLAST_SCORE_1MIN || hi > BLAST_SCORE_1MAX)
    return 1;

  if (hi - lo > BLAST_SCORE_RANGE_MAX) return 1;
 ...
 */
int BlastComputeLengthAdjustment(float K, float logK, float alpha_d_lambda,
                                 float beta, int query_length,
                                 unsigned int db_length, int db_num_seqs,
                                 int* length_adjustment) {
  int i;                 /* iteration index */
  const int maxits = 20; /* maximum allowed iterations */
